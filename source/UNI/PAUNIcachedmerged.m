%function [selected_PAUNImerged, time_PAUNImerged, window_trend] = PAUNIcachedmerged(dataindex, k, b, M, beta, gamma)
...
%Initialization%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
time_PAUNImerged = [0];
selected_PAUNImerged = []; %keep the selected feature subset
...
selected_PAUNImerged = [selected_PAUNImerged current];
...
previous_selected = selected_PAUNImerged; %this keeps track of selected subset previous step
...
while size(selected_PAUNImerged,2) <= k %select k or more features  
    ...
    parfor i = 2:M+1   
        ...
        for s = 1:i-1 %find the boundary of the interfering zone 
            MI = mi(data(:,sorted_features(i)),data(:,sorted_features(s)));
            CMI = cmi(data(:,sorted_features(i)),data(:,sorted_features(s)),y);
