%function [selected_PAJMImerged, time_PAJMImerged, window_trend] = PAJMILcachedmerged(dataindex, k, b, M, selected_JMI, time_JMI, dist_feature_set, score_dist_features, reles)

%%This code is also taking the warmstart as context variables: 'selected_JMI, time_JMI, dist_feature_set, score_dist_features, reles'
...
%Initialization%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
time_PAJMImerged = time_JMI;
selected_PAJMImerged = selected_JMI; %keep the selected feature subset
feature_set = [1:f]; %To get rid of the all-zero columns


parpool(M); %start the parallel pool and distributing the feature space
poolobj = gcp;
...
while size(selected_PAJMImerged,2) <= k %select k or more features  
        ...
        for p = features
            ...
            if p ~= 0 %we denote the selected/unavailable features as 0
                for q = previous_selected
                   temp = temp + mi(joint([full(data(:,p)),full(data(:,q))],[]),y);
                end
    ...
    parfor i = 2:M+1   %find the interfering zone
        ...
        for s = 1:i-1 %find the boundary of the interfering zone 
            temp = mi(joint([full(data(:,sorted_features(i))),full(data(:,sorted_features(s)))],[]), y);
            pre = [pre temp];
            boundary = boundary + temp - (reles(find(feature_set == sorted_features(s))) + 2*maximalmi);                %h(y); 
        end
    ...
    parfor i = 1:M
        for j = i+1:windows
            temp = mi(joint([full(data(:,sorted_features(i))),full(data(:,sorted_features(j)))],[]), y);
    ...
    time_PAJMImerged(size(selected_PAJMImerged,2)) = toc(start) + time_JMI(size(time_JMI, 2));
end
